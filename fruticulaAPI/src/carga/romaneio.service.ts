    ...
    return new Promise((resolve, reject) => {
      ...
      const drawTableHeader = (y: number): number => {
        ...
        for (let i = 0; i < lados.length; i++) {
          ...
          doc.text(labelLado[lados[i]], cx + PAD, y + 7, {
            width: COL_W - PAD * 2,
            align: 'center',
            lineBreak: false,
          });
      ...
      doc.text(`Romaneio ${motorista} ${dataHifen}`, ML, 30, {
        width: TW,
        align: 'center',
        lineBreak: false,
      });
      ...
      for (let bloco = 1; bloco <= qtdBlocos; bloco++) {
        ...
        doc.text(`${bloco}`, ML + PAD, y + (rH - 10) / 2, {
          width: BLOCO_W - PAD * 2,
          align: 'center',
          lineBreak: false,
        });
      ...
      const drawSumHeader = (y: number): number => {
        ...
        doc.text('Descrição do Sumário', ML + PAD, y + 5, {
          width: DESC_W - PAD * 2,
          lineBreak: false,
        });
        doc.text('Quantidade', ML + DESC_W + PAD, y + 5, {
          width: QTD_W - PAD * 2,
          align: 'center',
          lineBreak: false,
        });
      ...
      const drawRow = (
        ...
        doc.text(desc, ML + PAD, ys + 5, {
          width: DESC_W - PAD * 2,
          align: alignRight ? 'right' : 'left',
          lineBreak: false,
        });
        if (qty) {
          doc.text(qty, ML + DESC_W + PAD, ys + 5, {
            width: QTD_W - PAD * 2,
            lineBreak: false,
          });
        }
      ...
      // Total pallets row
      drawRow(
        'Total de Pallets na Carga',
        String(carga.pallets?.length ?? 0),
        true,
        WHITE,
      );
      ...
      for (const p of carga.pallets ?? []) {
        for (const pf of p.palletFrutas ?? []) {
          const nomeFruta =
            pf.tipoFrutaEmbalagem?.tipoFruta?.fruta?.nome ?? '—';
      ...
      const grupoFrutaOrdenado = Array.from(grupoFruta.entries()).sort(
